...
class InvoiceTests: XCTestCase {
    ...
    func createInvoice(address: String, network: Network, decodedPaymentRequest: PaymentRequest, testAssertions: @escaping (Result<Invoice>) -> Void) {
        ...
        Invoice.create(from: address, lightningService: mockService) {
                testAssertions($0)
                expectation.fulfill()
        }
